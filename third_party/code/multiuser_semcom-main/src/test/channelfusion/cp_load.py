...
from ... import utils
from ...train.train_CIF import get_best_checkpoint
...
from ...train.train_CIF import get_chfusion_trainer, get_MFNet_trainer

def get_trainer_from_checkpoint(logger, checkpoint_path: Path, args_path: Path, before_epoch: int=None, **kwargs) -> BaseTrainer:
    """
    ...
    return get_chfusion_trainer(**trainer_args)

def get_MFNet_trainer_from_checkpoint(logger, checkpoint_path: Path, args_path: Path, before_epoch: int=None, **kwargs) -> BaseTrainer:
    if checkpoint_path.is_dir():
        checkpoint_path = get_best_checkpoint(checkpoint_path, before_epoch)
    ...
    return get_MFNet_trainer(**trainer_args)