...
parser = argparse.ArgumentParser(
    description="Run SemCom images through DUSt3R."
)
...
snr_directory = f"snr_{snr_db:.1f}"
snr_label = f"{snr_db:g}"

experiment_name = (
    f"omdma_{channel_name}_snr{snr_label}"
)

bridge_root = Path(
    "/tmp2/b12504107/semcom-dust3r-bridge/"
    "reconstructed/omdma_chateau_common_seed42"
)
...
with timer("07_result_readback", use_cuda=True):
    ...
    # SemCom 重建影像可能使 DUSt3R confidence 降低。
    # exp confidence mode 的最低值約為 1，因此先使用 1.0。
    print("Default confidence threshold:", scene.min_conf_thr)
    scene.min_conf_thr = 1.0
    print("Evaluation confidence threshold:", scene.min_conf_thr)
...
#新增：檢查每張影像有多少有效點
for view_index, (points, mask, depth, image) in enumerate(view_data):
    geometric_valid = (
    ...
    final_valid = mask & geometric_valid

    print(
        f"View {view_index}: "
        f"mask points={int(mask.sum())}, "
        f"geometric points={int(geometric_valid.sum())}, "
        f"final valid points={int(final_valid.sum())}"
    )
...
with timer("10_baseline_export"):
    ...
    baseline_cloud.export(
        output_dir / f"{experiment_name}_standard.ply"
    )
    baseline_cloud.export(
        output_dir / f"{experiment_name}_standard.glb"
    )
...
with timer("11_adaptive_export"):
    ...
    adaptive_cloud.export(
        output_dir / f"{experiment_name}_depth_adaptive.ply"
    )
    adaptive_cloud.export(
        output_dir / f"{experiment_name}_depth_adaptive.glb"
    )
